...
exec 9>"${LOCK_FILE}"
if ! flock -n 9; then
  echo "$(date -Is) another BTCUSD moving-average update is already running; exiting"
  exit 0
fi

cd "${REPO_DIR}"

echo "$(date -Is) starting BTCUSD moving-average update"
...
if git diff --cached --quiet; then
  ...
  git config user.name "btc-ma-bot"
  git config user.email "btc-ma-bot@users.noreply.github.com"
  git commit -m "Update BTCUSD moving-average data"
  git push
  echo "$(date -Is) pushed BTCUSD moving-average data update"
fi

# Send an email only when the newest moving-average crossover is newer than the most recent
...
echo "$(date -Is) finished BTCUSD moving-average update"
