...
# Regenerates KGLD/IGLD/IAUI gold ETF comparison data, commits it if changed, and pushes to GitHub.
...
exec 9>"${LOCK_FILE}"
if ! flock -n 9; then
  echo "$(date -Is) another KGLD/IGLD/IAUI performance update is already running; exiting"
  exit 0
fi

cd "${REPO_DIR}"

echo "$(date -Is) starting KGLD/IGLD/IAUI performance update"
...
if git diff --cached --quiet; then
  echo "$(date -Is) no KGLD/IGLD/IAUI data changes to commit"
else
  ...
  git push
  echo "$(date -Is) pushed KGLD/IGLD/IAUI performance data update"
fi

echo "$(date -Is) finished KGLD/IGLD/IAUI performance update"
